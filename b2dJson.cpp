...
* modified by abakobo for box2d 2.4.1
*
    ...
    case e_distanceJoint:
        {
            ...
            floatToJson("stiffness", distanceJoint->GetStiffness(), jointValue);
            floatToJson("damping", distanceJoint->GetDamping(), jointValue);
        }
        break;
    case e_pulleyJoint:
        {
    ...
    case e_mouseJoint:
        {
            ...
            floatToJson("stiffness", mouseJoint->GetStiffness(), jointValue);
            floatToJson("damping", mouseJoint->GetDamping(), jointValue);
        }
        break;
    case e_gearJoint:
        {
    ...
    case e_wheelJoint:
        {
            ...
            floatToJson("stiffness", wheelJoint->GetStiffness(), jointValue);
            floatToJson("damping", wheelJoint->GetDamping(), jointValue);
        }
        break;
    case e_motorJoint:
        {
    ...
    case e_weldJoint:
        {
            ...
            floatToJson("stiffness", weldJoint->GetStiffness(), jointValue);
            floatToJson("damping", weldJoint->GetDamping(), jointValue);
        }
        break;
...
b2Body* b2dJson::j2b2Body(b2World* world, Json::Value &bodyValue)
{
    ...
    bodyDef.enabled = bodyValue.get("enabled",true).asBool();
...
b2Fixture* b2dJson::j2b2Fixture(b2Body* body, Json::Value &fixtureValue)
{
    ...
    if ( !fixtureValue["circle"].isNull() ) {
        ...
        edgeShape.m_oneSided = fixtureValue["edge"].get("onesided",false).asBool();
        ...
        b2Vec2 prevV=jsonToVec("prevVertex", fixtureValue["chain"]);
        b2Vec2 nextV=jsonToVec("nextVertex", fixtureValue["chain"]);
        
        if (prevV == vertices[numVertices - 2] and nextV==vertices[1])
            // is loop
            chainShape.CreateLoop( vertices, numVertices );
            
        else
            chainShape.CreateChain( vertices, numVertices , prevV, nextV );
...
b2Joint* b2dJson::j2b2Joint(b2World* world, Json::Value &jointValue)
{
    ...
    else if ( type == "distance" )
    {
        ...
        distanceDef.stiffness = jsonToFloat("stiffness", jointValue);
        distanceDef.damping = jsonToFloat("damping", jointValue);
    }
    else if ( type == "pulley" )
    {
        jointDef = &pulleyDef;
    ...
    else if ( type == "mouse" )
    {
        ...
        mouseDef.stiffness = jsonToFloat("stiffness", jointValue);
        mouseDef.damping = jsonToFloat("damping", jointValue);
    }
    else if ( type == "gear" )
    {
        jointDef = &gearDef;
    ...
    else if ( type == "wheel" )
    {
        ...
        wheelDef.stiffness = jsonToFloat("stiffness", jointValue);
        wheelDef.damping = jsonToFloat("damping", jointValue);
    }
    else if ( type == "motor" )
    {
        jointDef = &motorDef;
    ...
    else if ( type == "weld" )
    {
        ...
        weldDef.stiffness = jsonToFloat("stiffness", jointValue);
        weldDef.damping = jsonToFloat("damping", jointValue);
    }
